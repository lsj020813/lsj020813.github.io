...
description: "DeepRVAT annotation 문서를 읽으며, synonymous variant와 splicing motif를 variant weight 설계에서 어떻게 봐야 할지 정리한 노트."
math: true
---

DeepRVAT의 [annotation 문서](https://github.com/PMBio/deeprvat/blob/main/docs/annotations.md)를 읽다가, rare variant에 어떤 weight를 줄지 고민하게 됐다. 처음에는 `synonymous`, `missense`, `splice-disrupting`, `protein-truncating` 같은 용어를 영향 크기의 순서처럼 받아들이기 쉽다. 하지만 이 용어들은 같은 기준에서 나온 분류가 아니다.

이 글은 `splicing motif`를 중심으로, `synonymous variant`가 왜 항상 "조용한 변이"라고 볼 수 없는지 정리한 기록이다.

## 먼저 축을 나누기

variant consequence를 한 줄로 세워 놓으면 헷갈린다. 그래서 먼저 "무엇을 보고 붙인 이름인가"를 기준으로 나눠 봤다.
...
이 구분이 중요한 이유는 `splice-disrupting`과 `protein-truncating`이 같은 층의 말이 아니기 때문이다. `splice-disrupting`은 RNA 처리 과정에서 생긴 문제를 가리키고, `protein-truncating`은 그 결과로 protein이 짧아지거나 기능을 크게 잃을 가능성을 가리킨다. 어떤 splice-disrupting variant는 PTV처럼 작동할 수 있지만, 모든 splice-disrupting variant가 PTV인 것은 아니다.

## DNA에서 protein까지

유전정보의 흐름은 크게 다음처럼 이어진다.
...
여기서 주의할 점은 exon이 곧 protein-coding sequence는 아니라는 것이다. exon은 mature RNA에 남는 구간을 뜻한다. 그 안에는 coding sequence뿐 아니라 UTR 같은 비번역 구간도 들어갈 수 있다.

## Splicing motif는 RNA 편집 좌표계다

splicing motif를 이름 목록으로만 외우면 금방 헷갈린다. 내가 이해한 핵심은 이렇다.

> splicing motif는 pre-mRNA에서 어디를 자르고, 어디를 남기고, 어디를 붙일지 정하는 신호다.

세포는 긴 pre-mRNA를 두고 다음 질문에 답해야 한다.
...
이 질문에 맞춰 splicing signal을 네 층으로 나눠 볼 수 있다.
...
많은 intron은 `GU-AG` 형태의 핵심 신호를 가진다. 하지만 splicing은 단순히 `GU-AG`를 찾는 과정이 아니다. genome에는 비슷하게 생긴 가짜 splice site도 많고, 실제 splice site도 강한 것과 약한 것이 섞여 있다.

그래서 중요한 질문은 "GU-AG가 있는가?"에서 끝나지 않는다. 더 중요한 질문은 "이 위치가 실제로 spliceosome에게 선택될 만큼 충분한 문맥과 조절 신호를 갖는가?"다.
...
그래서 `branch point`는 경계 신호라기보다 반응 신호에 가깝다. intron의 시작과 끝을 알려주는 표지가 아니라, intron 제거 반응을 진행하기 위한 내부 기준점이다.

## 문맥 신호: PPT

`3' splice site`는 `AG` 같은 짧은 신호만으로 안정적으로 인식되기 어렵다. 그래서 그 주변에는 추가적인 문맥 신호가 필요하다.
...
`polypyrimidine tract`, 즉 PPT는 C 또는 U가 많이 모여 있는 구간이다. PPT가 직접 반응을 수행한다기보다는, `3' splice site`가 안정적으로 인식되도록 도와준다고 이해하는 편이 자연스럽다.

## 조절 신호: enhancer와 silencer

여기까지가 기본 좌표라면, 실제 splicing은 여기서 한 단계 더 나아간다. 세포는 다음도 판단해야 한다.
...
enhancer와 silencer는 splicing을 직접 수행하는 가위가 아니다. spliceosome이 어떤 exon 또는 splice site를 선택할지 조절하는 신호에 가깝다.
...
따라서 enhancer는 항상 좋은 신호이고 silencer는 항상 나쁜 신호라고 볼 수 없다. enhancer는 포함 쪽으로 미는 신호이고, silencer는 제외 쪽으로 미는 신호다. 정상적인 splicing에는 둘 다 필요하다. 문제가 되는 것은 variant 때문에 이 균형이 깨지는 경우다.
...
하지만 같은 염기서열이 항상 한 가지 역할만 하는 것은 아니다.
...
결국 synonymous variant는 amino acid code를 바꾸지 않더라도, splicing code를 바꿀 수 있다.

## DeepRVAT annotation과 weight 설계로 연결하기

DeepRVAT의 annotation 과정은 variant를 모델에 넣기 전에 여러 층의 정보를 붙이는 단계라고 볼 수 있다. VEP 같은 codon-level consequence만 보는 것이 아니라, SpliceAI, DeepRiPe, deepSEA, abSplice 같은 annotation을 함께 고려하는 이유도 여기에 있다.
...
| splice-disrupting이면 곧 PTV다 | splicing disruption은 원인에 가깝고, PTV는 결과에 가깝다 |
| GU-AG만 보면 된다 | splice site 선택은 경계, 반응, 문맥, 조절 신호의 조합으로 결정된다 |

따라서 variant weight를 consequence label 하나만으로 정하기는 어렵다. 적어도 다음 층은 나눠서 보는 편이 낫다.
...
이렇게 보면 synonymous variant에 무조건 낮은 weight를 주는 것은 항상 타당하지 않다. 반대로 intronic variant라고 해서 무조건 버리는 것도 위험하다. 중요한 것은 variant가 어떤 좌표계의 어떤 신호를 건드리는지다.

## 정리

splicing motif는 단순한 이름 목록이 아니다. pre-mRNA에서 intron/exon 경계와 exon 포함 여부를 결정하는 분산된 좌표계에 가깝다.
...
synonymous variant는 amino acid code를 바꾸지 않을 수 있지만, splicing code는 바꿀 수 있다. 그래서 rare variant annotation에서 synonymous, intronic, splice-disrupting, protein-truncating 같은 표현은 하나의 등급표로 보기보다 서로 다른 축의 정보로 나눠서 보는 편이 좋다.
